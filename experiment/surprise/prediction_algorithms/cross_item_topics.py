...
class CrossItemTopics(AlgoBase):
    ...
    def construct_all_tags_set(self):
        ...
        '''
        self.all_tags_set = self.trainset.tags_set.union(
            self.aux_trainset.tags_set)
    ...
    def sgd(self, trainset, aux_trainset):
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r in aux_trainset.all_ratings():
                ...
                topic_prop = self.lda_model.doc_topic_[
                    i + self.trainset.n_items]
...
class CrossItemTopicsTest(AlgoBase):
    ...
    def sgd(self, trainset, aux_trainset):
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r in aux_trainset.all_ratings():
                ...
                # compute current error
                dot = np.dot((aux_qi[i] + topic_prop), aux_pu[u])
                err = r - (aux_global_mean + aux_bu[u] + aux_bi[i] + dot)
                ...
                # update factors
                aux_pu[u] += lr_all * \
                    (err * (aux_qi[i] + topic_prop) - reg_all * aux_pu[u])
                aux_qi[i] += lr_all * (err * aux_pu[u] - reg_all * aux_qi[i])
