...
class UserItemRelTags(AlgoBase):
...
class UserItemGenomeTags(AlgoBase):
    ...
    def sgd(self, trainset):
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r, tags_score in trainset.all_ratings_genome_tags_score():
                ...
                weight_sum = max(sum([tags_score.values()]), 1)
                sum_weighted_yt = np.sum(
                    [yt[tid] * score for tid, score in tags_score], axis=0) / weight_sum
                ...
                for t, score in tags_score:
                    yt[t] += lr_all * \
                        (pu[u] * score * (err / weight_sum) - reg_all * yt[t])
    ...
    def estimate(self, u, i, tags):
        ...
        if self.trainset.knows_user(u) and self.trainset.knows_item(i):
            ...
            weighted_yt_sum = yt_sum / max(1, weight_sum)

            est += np.dot((self.qi[i] + weighted_yt_sum), self.pu[u])
