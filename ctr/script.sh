...
#for i in `seq 1 5`
#do
##  ./ctr --directory $root_path/cv-cf-$i --user $root_path/cf-train-$i-users.dat --item \
...
#  for type in ofm cf
#  do
#  ./ctr --directory $root_path/cv-ctr-$i-$type --user $root_path/$type-train-$i-users.dat --item \
...
#     do
#       if [ "$lambda" == 100 ]; then
#       ./ctr --directory $rootpath/cv-in-matrix/lda-fold-$i-K-$K-lambda-$lambda \
...
#       ./ctr --directory $rootpath/cv-out-of-matrix/lda-fold-$i-K-$K-lambda-$lambda \
...
do
  for K in 50
  do 
    for lambda in 10 100 1000 10000
    do
      ./qsub.sh ./ctr --directory $rootpath/cv-in-matrix/ctr-fold-$i-K-$K-lambda-$lambda \
      --user $rootpath/cv-in-matrix/fold-$i-users.train \
      --item $rootpath/cv-in-matrix/fold-$i-items.train \
